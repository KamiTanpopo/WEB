...
<?php }
  ...
  document.getElementById('gray').style.display = state;
                ...
                <div class="EchoText"><h2>Сеанси фільмів на 15.12</h2></div>
                <?php 
                    $sql="SELECT    films.film_name, films.director, films.genre, films.country, films.actors, films.duration, films.rating, 
                                    session.date, session.prise, session.idsession 
                    FROM `films` INNER JOIN `session`
                    WHERE films.idfilm = session.film_name AND session.date LIKE '%2021-12-15%'
