...
<?php }
  ...
  document.getElementById('gray').style.display = state;
                ...
                <div class="EchoText"><h2>Купівля квитка на сеанс</h2></div>
                <?php 
                    $sessionId = substr(parse_url($_SERVER[REQUEST_URI])["query"], 3);
                    $sql="  SELECT films.film_name,  session.date, session.prise, halls.hall_name, session.ticketsnum
                            FROM `films` 	INNER JOIN `session` ON films.idfilm = session.film_name
                                            INNER JOIN `halls` ON idhalls = hall
                            WHERE session.idsession = $sessionId
                            ORDER BY session.date";
                    $films=mysqli_query($connect,$sql);
                    $films = mysqli_fetch_all($films);
                ?>

                <table class="table" id ="f">
                    <tr>
                        <th>Назва фільму</th>
                        <th>Дата</th>
                        <th>Ціна</th>
                        <th>Зала</th>
                        <th>Кількість вільних білетів</th>
                    </tr> 
                    <?php    
                ...
                <?php 
                    $sessionId = parse_url($_SERVER[REQUEST_URI]);
                    echo '
                        <form id="forma" action="Buying1.php?'.$sessionId["query"].'" method ="post" class="forma">
                            <p>Ваше Імя<input type="text" name="firstName"></p>
                            <p>Ваше Прізвище<input type="text" name="lastName"></p>
                            <p>Кількість білетів<input type="text" name="numOfTickets"></p>
                            <input style="visibility:hidden" type="text" name="price" value="'.$film[2].'"></input>
                            <input type="submit" value="Купити!" class="subm">
                        </form>
                        ';
                ?>
        </header>
        <script src ="cite_js.js"> </>
    </body>
</html>